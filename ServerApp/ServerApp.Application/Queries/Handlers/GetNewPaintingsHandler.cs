...
public class GetNewPaintingsHandler : IRequestHandler<GetNewPaintings, List<PaintingDto>>
{
    private readonly IPaintingReadRepository _readRepository;

    public GetNewPaintingsHandler(IPaintingReadRepository readRepository)
    {
        _readRepository = readRepository;
    }

    public async Task<List<PaintingDto>> Handle(GetNewPaintings query, CancellationToken cancellationToken = default)
    {
        var paintings = await _readRepository.GetAllAsync(cancellationToken);

        return paintings
            .Where(p => p.IsNew.Value)
